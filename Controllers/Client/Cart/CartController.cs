...
namespace GameStore.Controllers.Client.Cart
{
    ...
    public class CartController : Controller
    {
        ...
        // Key lưu mã khuyến mãi đang áp trong session
        private const string CouponKey = "COUPON_MAKM";
        ...
        public async Task<IActionResult> Index(string returnUrl)
        {
            ...
            // ===== Mã khuyến mãi đang áp (đọc từ session) =====
            decimal total = cart.ChiTietGioHangs.Sum(x => x.DonGiaHienTai);
            decimal discount = 0;
            KhuyenMai? applied = null;
            var makm = HttpContext.Session.GetInt32(CouponKey);
            if (makm.HasValue)
            {
                var km = couponService.GetUsableById(activeUser.MaNguoiDung, makm.Value);
                if (km != null && (km.DonToiThieu == null || total >= km.DonToiThieu.Value))
                {
                    applied = km;
                    discount = couponService.ComputeDiscount(km, total);
                }
                else
                {
                    // mã hết hạn / đã dùng / không đủ điều kiện -> bỏ áp
            ...
            ViewBag.MyCoupons = couponService.GetMyUsableCoupons(activeUser.MaNguoiDung);
        ...
        // Áp mã khuyến mãi vào giỏ (lưu vào session)
        [HttpPost("apply-coupon")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ApplyCoupon(string code, string returnUrl = null)
        {
            var activeUser = await GetCurrentActiveUserAsync();
            if (activeUser == null)
                return Redirect("/auth/login");
            ...
            var cart = cartService.GetCart(activeUser.MaNguoiDung);
            decimal total = cart?.ChiTietGioHangs.Sum(x => x.DonGiaHienTai) ?? 0;

            var (ok, message, _, km) = couponService.TryApply(activeUser.MaNguoiDung, code, total);
            if (ok && km != null)
                HttpContext.Session.SetInt32(CouponKey, km.MaKM);
        ...
        // Bỏ mã khuyến mãi đang áp
        [HttpGet("remove-coupon")]
        public async Task<IActionResult> RemoveCoupon(string returnUrl = null)
        {
            ...
            TempData["ToastMessage"] = "Đã bỏ mã khuyến mãi.";
            TempData["ToastType"] = "success";
            return RedirectToAction("Index", new { returnUrl });
        }

        // Lấy (maKM, discount) hợp lệ hiện tại từ session để áp khi thanh toán
        private (int? maKM, decimal discount) GetSessionCoupon(int userId, decimal total)
        {
            var makm = HttpContext.Session.GetInt32(CouponKey);
            if (!makm.HasValue) return (null, 0);

            var km = couponService.GetUsableById(userId, makm.Value);
            if (km == null || (km.DonToiThieu.HasValue && total < km.DonToiThieu.Value))
                return (null, 0);

            return (km.MaKM, couponService.ComputeDiscount(km, total));
        }
        ...
        public async Task<IActionResult> Checkout(string method = "balance", string returnUrl = null)
        {
            ...
            if (method.Equals("momo", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // 🔥 áp mã khuyến mãi (nếu có) -> tính số tiền cuối
                var (maKM, discount) = GetSessionCoupon(userId, total);
                var finalTotal = total - discount;
                if (finalTotal < 0) finalTotal = 0;

                var maGD = Guid.NewGuid().ToString();

                paymentService.CreatePendingMomo(userId, maGD, finalTotal, maKM, discount);

                var baseUrl = $"{Request.Scheme}://{Request.Host}";
                var paymentUrl = await momoService.CreatePaymentUrlForOrder(userId, maGD, finalTotal, baseUrl);
            ...
            var balanceCart = cartService.GetCart(userId);
            var balanceTotal = balanceCart?.ChiTietGioHangs.Sum(x => x.DonGiaHienTai) ?? 0;
            var (balMaKM, balDiscount) = GetSessionCoupon(userId, balanceTotal);

            var result = await paymentService.Checkout(userId, balMaKM, balDiscount);

            // Thanh toán bằng số dư thành công -> bỏ mã khỏi session (đã dùng)
            if (result)
                HttpContext.Session.Remove(CouponKey);
