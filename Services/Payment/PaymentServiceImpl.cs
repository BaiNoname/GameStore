...
namespace GameStore.Services
{
    public class PaymentServiceImpl : PaymentService
    {
        ...
        // Service xử lý mã khuyến mãi (đánh dấu đã dùng khi thanh toán thành công)
        ...
        public async Task<bool> Checkout(int userId, int? maKM = null, decimal discount = 0)
        {
            ...
            try
            {
                ...
                // 🔥 áp mã khuyến mãi: giảm giá không vượt quá tổng đơn
                if (discount < 0) discount = 0;
                if (discount > total) discount = total;
                decimal finalTotal = total - discount;

                logger.LogInformation("Checkout start: UserId={UserId}, BalanceBefore={BalanceBefore}, Total={Total}, Discount={Discount}",
                    userId, user.SoDu, total, discount);

                // ❌ không đủ tiền (so với số tiền sau giảm)
                if (user.SoDu < finalTotal)
                {
                ...
                // 🔥 trừ tiền sau khi tạo transaction (số tiền sau giảm)
                user.SoDu -= finalTotal;
                ...
                // 🔥 đánh dấu mã khuyến mãi đã dùng (nếu có)
                if (maKM.HasValue)
                {
                    var link = db.NguoiDungKhuyenMais
                        .FirstOrDefault(x => x.MaNguoiDung == userId && x.MaKM == maKM.Value);
                    if (link != null && !link.DaSuDung)
                    {
                        link.DaSuDung = true;
                        link.NgaySuDung = DateTime.UtcNow;
                        link.MaGD = giaoDich.MaGD;
                    }
                }
        ...
        public void CreatePendingMomo(int userId, string maGD, decimal amount, int? maKM = null, decimal discount = 0)
        {
            var user = GetActiveUser(userId);
            if (user == null)
            ...
            // tạo giao dịch với trạng thái Pending (lưu mã KM để đánh dấu đã dùng khi callback thành công)
            var giaoDich = new GiaoDich
            {
                MaGD = maGD,
                MaNguoiDung = userId,
        ...
        public async Task CompleteMomo(string maGD)
        {
            ...
            try
            {
                ...
                // 🔥 đánh dấu mã khuyến mãi đã dùng (nếu giao dịch có áp mã)
                if (gd.MaKM.HasValue)
                {
                    var link = db.NguoiDungKhuyenMais
                        .FirstOrDefault(x => x.MaNguoiDung == gd.MaNguoiDung && x.MaKM == gd.MaKM.Value);
                    if (link != null && !link.DaSuDung)
                    {
                        link.DaSuDung = true;
                        link.NgaySuDung = DateTime.UtcNow;
